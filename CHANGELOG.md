...
- New manager events: `worker_online`, `worker_exit`, `worker_restart_scheduled`, `worker_listening`, `scale_up`, `scale_down`, `reload_start`, `reload_end`, `reload_fail`, `shutdown_start`, `shutdown_end`.
...
- Metrics now report `uptime` from worker start time, and memory scale-up averaging uses only workers with memory samples.
